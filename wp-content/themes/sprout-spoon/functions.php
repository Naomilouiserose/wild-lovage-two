...
function solopine_load_scripts() {
	...
	wp_enqueue_script('print', get_template_directory_uri() . '/js/jQuery.print.js', array( 'jquery' ), '', true);
	wp_enqueue_script('fitvids', get_template_directory_uri() . '/js/fitvids.js', array( 'jquery' ), '', true);
	wp_enqueue_script('sprout_spoon_scripts', get_template_directory_uri() . '/js/solopine.js', array( 'jquery' ), '', true);
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
			...
			<div class="thecomment">
				...
				<div class="comment-text">
					<span class="reply">
						<?php comment_reply_link(array_merge( $args, array('reply_text' => esc_html__('Reply', 'sprout-spoon'), 'depth' => $depth, 'max_depth' => $args['max_depth'])), $comment->comment_ID); ?>
...
function solopine_pagination() {
	...
	<div class="pagination <?php if(get_theme_mod('sprout_spoon_home_layout') == 'grid2' || get_theme_mod('sprout_spoon_home_layout') == 'full_grid2' || get_theme_mod('sprout_spoon_home_layout') == 'grid3' || get_theme_mod('sprout_spoon_home_layout') == 'full_grid3') : ?>pagi-grid<?php endif; ?>">
		...
		<div class="newer"><?php previous_posts_link(wp_kses( __( '<i class="fas fa-angle-left"></i> Newer Posts', 'sprout-spoon' ), array( 'i' => array( 'class' => array() ) ) )); ?></div>
		<div class="older"><?php next_posts_link(wp_kses( __( 'Older Posts <i class="fas fa-angle-right"></i>', 'sprout-spoon' ), array( 'i' => array( 'class' => array() ) ) )); ?></div>
